        ...
        {   
            ...
            try
            {
                ...
                for (int i = 0; i < billDetailList.Count; i++)
                {
                    ...
                    ws.Cells[indexRow, 8].Value = $"{SupportBUS.formatPrice(_currentProduct.PriceToSell.ToString())}";
                    ws.Cells[indexRow, 8].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Right;

                    ws.Cells[indexRow, 9, indexRow, 10].Merge = true;
                    ws.Cells[indexRow, 9, indexRow, 10].Value = $"{SupportBUS.formatPrice((_currentProduct.PriceToSell*_currentDetail.Quantity).ToString())}      ";

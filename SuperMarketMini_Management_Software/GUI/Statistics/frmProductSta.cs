            ...
            if (rbtnDate.Checked)
                switch (cbDate.Text)
                {
                    case "Hôm nay":
                        ...
                        break;
                    case "Tháng này":
                        dt = statisticsBUS.getAllProductByFilter(txtSearch.Texts, cbCategory.Text, new DateTime(DateTime.Today.Year,DateTime.Today.Month,1), new DateTime(DateTime.Today.Year, DateTime.Today.Month, DateTime.DaysInMonth(DateTime.Today.Year, DateTime.Today.Month)), from, to);
                        break;
                    case "Năm nay":
                        dt = statisticsBUS.getAllProductByFilter(txtSearch.Texts,  cbCategory.Text, new DateTime(DateTime.Today.Year,1,1), new DateTime(DateTime.Today.Year, 12, 31), from, to);
                        break;
                    default:
